...
- [x] `internal/domain/post.go` 작성
    - [x] `Post` 구조체 정의 (ID, Title, Content, CreatedAt)
    - [x] JSON 태그 설정 (`json:"title"`)
    - [x] **학습 포인트**:
        - Class 대신 Struct 사용
...
- [x] `internal/repository/post_repository.go` 작성
    - [x] `PostRepository` 인터페이스 정의 (Save, FindByID, FindAll, Update, Delete)
    - [x] `memoryPostRepository` 구조체 구현 (Thread-safe를 위해 `sync.RWMutex` 활용)
    - [x] **학습 포인트**:
...
- [x] `internal/service/post_service.go` 작성
    - [x] `PostService` 구조체 정의 (Repository 인터페이스 주입)
    - [x] `NewPostService` 생성자 함수 작성 (DI)
    - [x] **학습 포인트**:
...
- [x] `internal/handler/post_handler.go` 작성
    - [x] `PostHandler` 구조체 정의 (Service 주입)
    - [x] CRUD 메서드 구현 (`Create`, `Get`, `List`, `Update`, `Delete`)
    - [x] Echo Context (`c echo.Context`) 활용
    - [x] **학습 포인트**:
...
- [x] `cmd/app/main.go` 수정
    - [x] Repository -> Service -> Handler 순서로 의존성 객체 생성 및 주입
    - [x] Echo 라우팅 그룹 설정 (`/posts`)
    - [x] **학습 포인트**:
...
- [x] DTO(Request/Response) 분리하여 엔티티 오염 방지