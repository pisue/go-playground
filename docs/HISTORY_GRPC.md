...
> 💡 **상세 가이드**: 구조체, 포인터, 데이터 흐름에 대한 쉬운 비유(은행 직원, 공구함 등)와 상세한 설명은 **[Go gRPC 개발 핵심 개념: 구조체와 상태 관리](./GO_GRPC_CONCEPTS.md)** 문서에 별도로 정리해 두었습니다.
...
### 3.5 Paseto 토큰 관리 로직 구현
Paseto(Platform-Agnostic SEcurity TOkens) 라이브러리를 활용하여 인증 토큰을 생성하고 검증하는 로직을 구체화했습니다.

*   **토큰 생성 (`CreateNewToken`)**:
    *   `crypto/rand` 패키지를 사용하여 16바이트 난수(Random Salt/Nonce)를 생성.
